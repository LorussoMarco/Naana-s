...
export default function Stepper({
  ...
  const tryUpdate = async (newStep: number) => {
    if (newStep > currentStep && onBeforeStepChange) {
      try {
        const allowed = await onBeforeStepChange(currentStep, newStep);
        if (!allowed) return;
      } catch (err) {
        return;
      }
  ...
  const handleNext = () => {
    if (!isLastStep) {
      void tryUpdate(currentStep + 1);
    }
  };

  const handleComplete = () => {
    void tryUpdate(totalSteps + 1);
  };
          ...
          <div className={`footer-container ${footerClassName}`}>
            <div className={`footer-nav ${currentStep !== 1 ? 'spread' : 'end'}`}>
              ...
              <button onClick={isLastStep ? handleComplete : handleNext} className="next-button" {...nextButtonProps}>
                {isLastStep ? 'Invia' : nextButtonText}
              </button>
            </div>
          </div>
        )}
